...
# На хостингах без загрузки файлов (например, Railway) можно вставить
# содержимое JSON-ключа сервисного аккаунта целиком в переменную окружения
# GOOGLE_CREDENTIALS_JSON — при старте бот сам сохранит его в файл.
...
def init_google_sheet() -> None:
    ...
    # Если файла ключа нет на диске, но задана переменная GOOGLE_CREDENTIALS_JSON —
    # создаём файл из неё (актуально для Railway и подобных хостингов).
    if not os.path.exists(GOOGLE_CREDENTIALS_FILE) and GOOGLE_CREDENTIALS_JSON:
        try:
            ...
            logger.info(
                "Файл ключа сервисного аккаунта создан из GOOGLE_CREDENTIALS_JSON: %s",
                GOOGLE_CREDENTIALS_FILE,
            )
        except Exception:
            logger.exception(
                "Не удалось создать файл ключа из GOOGLE_CREDENTIALS_JSON"
            )

    if not os.path.exists(GOOGLE_CREDENTIALS_FILE):
        logger.error(
            "Файл ключа сервисного аккаунта не найден: %s — запись в таблицу "
            "работать не будет. Задайте GOOGLE_CREDENTIALS_FILE (путь к файлу) "
            "или GOOGLE_CREDENTIALS_JSON (содержимое ключа целиком).",
            GOOGLE_CREDENTIALS_FILE,
        )
        return
